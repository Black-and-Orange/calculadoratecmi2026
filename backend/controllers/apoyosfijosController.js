const apoyosModel = require('../models/apoyosFijos');
const apoyosNivelModel = require('../models/apoyosFijosNivel');
...
const createApoyoWithNivel = (req, res) => {
    const { valor, nivel_id } = req.body;

    // Crea el apoyo
    apoyosModel.createApoyo({ valor }, (err, result) => {
        if (err) return res.status(500).json({ error: err.message });
...
const updateApoyoWithNivel = (req, res) => {
    ...
    const allowedApoyoUpdates = ['valor'];
